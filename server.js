...
app.get('/api/inventory/type/:name', (req, res) => {
	var s = req.params.name
	handle_Type_Api(res, s)
});
// Routing------------------------------------------------------------------------------------------------------
...
const handle_Details = (res, criteria) => {
	...
	db.once('open', () => {
		...
		Booking.findOne(DOCID, (err, docs) => {
			if (err) return console.error(err);
			if (docs.inventory_address.coord != null && docs.inventory_address.coord != '') {
				var s = docs.inventory_address.coord.split(",")
			}
...
const handle_Type_Api = (res, criteria) => {
	mongoose.connect(mongourl);
	let db = mongoose.connection;
	db.on('error', console.error.bind(console, 'connection error:'));
	db.once('open', () => {
...
const handle_Create = (res, criteria) => {
	...
	db.once('open', () => {
		...
		if (criteria.body.latitude != null && criteria.body.longitude != null && criteria.body.latitude != '' && criteria.body.longitude != '')
			latitudeAndLongitude = criteria.body.latitude + "," + criteria.body.longitude;
		else {
		...
		Booking.findOne(DOCID, (err, docs) => {
			...
			if (docs.manager != criteria.session.username) {
				...
				if (criteria.body.latitude != null && criteria.body.longitude != null && criteria.body.latitude != '' && criteria.body.longitude != '')
					...
					docs.inventory_address.coord = null
				}
